...

{
    ...
    // Convert roughness to specular exponent value, with safety check if
    // roughness == 0.0
    ...
    if (RepeatS != 0.0)
    {
        ss = mod(s * RepeatS, 1.0);
    }
    if (RepeatT != 0.0)
    {
        tt = mod(t * RepeatT, 1.0);
    }
    ...
    if (ColMap != "")
    {
        ColorTex = texture(ColMap, ss, tt);
    }

    color ColorSpec = 1.0;
    if (SpecMap != "")
    {
        ColorSpec = texture(SpecMap, ss, tt);
    }
    ...
    if (ColMap != "")
    {
        ...
        if (textureinfo(ColMap, "channels", numberOfChannels))
        {
            if (numberOfChannels == 4)
            {
                Oi = float texture(ColMap[3], s, t);
                Ci *= Oi;
            }
        }
    }
